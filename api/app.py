...
# BE-19 — Why-this explanation endpoint
try:
    from api.routes import explainer as explainer_route
    _EXPLAINER_ROUTER_OK = True
except Exception as _e:
    logger.error("Failed to import explainer router (BE-19): %s", _e)
    _EXPLAINER_ROUTER_OK = False
...
def create_app() -> FastAPI:
    ...
    if _EXPLAINER_ROUTER_OK:
        all_routers.append(explainer_route.router)
