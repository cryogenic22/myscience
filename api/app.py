...
# DataHub D-API-1 — connector taxonomy + onboarding lifecycle (own /hub prefix)
try:
    from api.routes import hub as hub_route
    _HUB_ROUTER_OK = True
except Exception as _e:
    logger.error("Failed to import hub router (D-API-1): %s", _e)
...
def create_app() -> FastAPI:
    ...
    if _HUB_ROUTER_OK:
        all_routers.append(hub_route.router)         # D-API-1 — /hub (own prefix)
