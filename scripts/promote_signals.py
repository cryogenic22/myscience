...
from services.signal_promoter import promote_events, build_signal_row

logger = logging.getLogger(__name__)


def run(*, limit: int = 100000, since_days: int | None = None, dry_run: bool = False) -> dict:
    ...
    try:
        if dry_run:
            ...
            events = db.fetch_all(
                """SELECT id, event_type, description, source_tier, trust_score,
            ...
            would_promote = would_skip_existing = would_skip_no_entity = 0
            for e in events:
                if str(e["id"]) in existing:
                    would_skip_existing += 1
                elif build_signal_row(e) is None:
            ...
            res = promote_events(db, limit=limit, since_days=since_days)
            summary = {
                "scanned": res.scanned,
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Promote market_events into candidate signals.")
    ap.add_argument("--limit", type=int, default=100000)
    ap.add_argument("--since-days", type=int, default=None)
    ap.add_argument("--dry-run", action="store_true")
    args = ap.parse_args()
    run(limit=args.limit, since_days=args.since_days, dry_run=args.dry_run)
